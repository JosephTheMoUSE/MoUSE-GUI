...
def generate_app_model(temporary_path: Path):
    ...
    with mock.patch(
        ...
        with mock.patch(
                "mouseapp.model.main_models.appdirs.user_data_dir",
                mock.MagicMock(return_value=str(temporary_path)),
        ):
...
def project_model(path_tmpdir):
    ...
    project_model._project_path = path_tmpdir.joinpath(project_model.project_name)
...
def clean_main_model(clean_app_model, clean_project_model, clean_spec_model):
    return MainModel(
        application_model=clean_app_model,
