...
def test_project_tab_updates(qtbot, main_model, empty_main_model):
    ...
    main_model.project_model.audio_files = [
        Path("/some/path/file.wav"), Path("/some/path/folder/file.wav")
    ]
    ...
    qtbot.mouseClick(new_project_window.load_audio_widget.buttonFinish,
                     qt_api.QtCore.Qt.MouseButton.LeftButton)
    files_in_file_list = set(
        item.fileName.text()
        for item in project_tab.fileList.findChildren(FileName))
    ...
    assert project_tab.key_value_widgets["key-1"].valueEditField.toPlainText(
    ) == "value"
    ...
    assert project_tab.key_value_widgets["key-2"].valueEditField.toPlainText(
    ) == "4.2"
    ...
    assert project_tab.key_value_widgets["key3"].valueEditField.toPlainText(
    ) == "1"
...
def test_project_update_on_change(qtbot, main_model):
    ...
    qtbot.keyClicks(project_tab.key_value_widgets["key3"].valueTypeComboBox,
                    "Real")
...
def test_no_change_on_wrong_type_selected(qtbot, main_model):
    ...
    qtbot.keyClicks(project_tab.key_value_widgets["key-1"].valueTypeComboBox,
                    "Integer")
