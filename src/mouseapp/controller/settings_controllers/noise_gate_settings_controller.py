...
from mouseapp.controller.utils import warn_user, run_background_task, \
    float_convert
...
def load_noise_audio(model: MainModel, file: Path):
    if model.spectrogram_model.spectrogram_calculator is None:
        warn_user(model,
                  "Before loading audio with noise, load main recordings")
        return
    ...
    model.settings_model.noise_gate_model.set_noise_audio_file_passive_signal(
        file)
...
def check_change_preconditions(model: MainModel):
    if model.settings_model.noise_gate_model.noise_audio_file is None \
            and not model.settings_model.noise_gate_model.use_main_spectrogram:
...
def set_noise_gate_preview(model: MainModel):
    ...
    def set_preview():
        ...
        denoising_controller. \
            apply_noise_gate_filter(model=model,
